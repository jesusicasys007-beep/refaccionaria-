...
<div class="flex items-center justify-between gap-4 mb-6">
    <div>
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Crear OrderItem</h1>
    ...
    <form method="POST" action="{{ route('admin.orderitems.store') }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
                ...
                <select name="order_id" id="order_id" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-slate-800 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none transition-all" required>
                    ...
                    @foreach($orders as $rel)
                        <option value="{{ $rel->id }}" {{ old('order_id') == $rel->id ? 'selected' : '' }}>{{ $rel->order_number ?? $rel->id }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label for="item_id" class="block text-sm font-semibold text-slate-700 mb-2">Item id</label>
                <input type="text" name="item_id" id="item_id" value="{{ old('item_id') }}" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-slate-800 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none transition-all placeholder:text-slate-400"   placeholder="Ingrese Item id...">
            </div>
            <div>
                <label for="item_type" class="block text-sm font-semibold text-slate-700 mb-2">Item type</label>
                <input type="text" name="item_type" id="item_type" value="{{ old('item_type') }}" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-slate-800 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none transition-all placeholder:text-slate-400"   placeholder="Ingrese Item type...">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="number" name="unit_price" id="unit_price" value="{{ old('unit_price') }}" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-slate-800 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none transition-all placeholder:text-slate-400"  step="any" placeholder="Ingrese Unit price...">
            </div>
            <div>
                <label for="total" class="block text-sm font-semibold text-slate-700 mb-2">Total</label>
                <input type="number" name="total" id="total" value="{{ old('total') }}" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-slate-800 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none transition-all placeholder:text-slate-400"  step="any" placeholder="Ingrese Total...">
            </div>
        </div>
