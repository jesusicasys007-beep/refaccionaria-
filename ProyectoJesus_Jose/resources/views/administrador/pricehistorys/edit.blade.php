...
<div class="flex items-center justify-between gap-4 mb-6">
    <div>
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Editar PriceHistory</h1>
...
<div class="bg-white border border-slate-100 shadow-sm rounded-2xl p-6 md:p-8">
    <form method="POST" action="{{ route('admin.pricehistorys.update', $pricehistory) }}">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
                <label for="priceable_id" class="block text-sm font-semibold text-slate-700 mb-2">Priceable id</label>
                <input type="number" name="priceable_id" id="priceable_id" value="{{ old('priceable_id', $pricehistory->priceable_id) }}" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-slate-800 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none transition-all placeholder:text-slate-400"  step="any">
            </div>
            <div>
                <label for="priceable_type" class="block text-sm font-semibold text-slate-700 mb-2">Priceable type</label>
                <input type="number" name="priceable_type" id="priceable_type" value="{{ old('priceable_type', $pricehistory->priceable_type) }}" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-slate-800 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none transition-all placeholder:text-slate-400"  step="any">
            </div>
            <div>
                <label for="price" class="block text-sm font-semibold text-slate-700 mb-2">Price</label>
                <input type="number" name="price" id="price" value="{{ old('price', $pricehistory->price) }}" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-slate-800 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none transition-all placeholder:text-slate-400"  step="any">
            </div>
            <div>
                <label for="currency" class="block text-sm font-semibold text-slate-700 mb-2">Currency</label>
                <input type="text" name="currency" id="currency" value="{{ old('currency', $pricehistory->currency) }}" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-slate-800 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none transition-all placeholder:text-slate-400"  >
            </div>
            <div>
                ...
                <select name="user_id" id="user_id" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-slate-800 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none transition-all" required>
                    ...
                    @foreach($users as $rel)
                        <option value="{{ $rel->id }}" {{ old('user_id', $pricehistory->user_id) == $rel->id ? 'selected' : '' }}>{{ $rel->name ?? $rel->id }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label for="effective_at" class="block text-sm font-semibold text-slate-700 mb-2">Effective at</label>
                <input type="text" name="effective_at" id="effective_at" value="{{ old('effective_at', $pricehistory->effective_at) }}" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-slate-800 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none transition-all placeholder:text-slate-400"  >
            </div>
        </div>
