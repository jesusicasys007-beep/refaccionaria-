...
<div class="flex items-center justify-between gap-4 mb-6">
    <div>
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Editar Image</h1>
    ...
    <form method="POST" action="{{ route('admin.images.update', $image) }}" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
                ...
                @if($image->path)
                    <div class="mb-3">
                        <img src="{{ asset('storage/' . $image->path) }}" class="w-32 h-32 object-cover rounded-xl border shadow-sm">
                    </div>
                @endif
            ...
            <div>
                ...
                <input type="text" name="imageable_id" id="imageable_id" value="{{ old('imageable_id', $image->imageable_id) }}" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-slate-800 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none transition-all placeholder:text-slate-400"  >
            </div>
            <div>
                <label for="imageable_type" class="block text-sm font-semibold text-slate-700 mb-2">Imageable type</label>
                <input type="text" name="imageable_type" id="imageable_type" value="{{ old('imageable_type', $image->imageable_type) }}" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-slate-800 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none transition-all placeholder:text-slate-400"  >
            </div>
            <div>
                <label for="alt" class="block text-sm font-semibold text-slate-700 mb-2">Alt</label>
                <input type="text" name="alt" id="alt" value="{{ old('alt', $image->alt) }}" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-slate-800 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none transition-all placeholder:text-slate-400"  >
            </div>
            <div>
                <label for="order" class="block text-sm font-semibold text-slate-700 mb-2">Order</label>
                <input type="text" name="order" id="order" value="{{ old('order', $image->order) }}" class="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-slate-800 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:outline-none transition-all placeholder:text-slate-400"  >
            </div>
        </div>
